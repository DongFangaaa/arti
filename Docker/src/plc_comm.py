...
class TCP:
    ...
    def start(self, should_stop=None) -> tuple[str, int]:
        """启动监听并等待 PLC 连接；连接成功后返回 PLC 的地址。

        accept 采用 0.5s 超时轮询：传入 should_stop() 返回 True 时
        中止等待并抛 ConnectionError（用于容器停止 / Ctrl-C 优雅退出）。
        """
        ...
        server.listen(1)
        server.settimeout(0.5)   # 超时轮询，避免 accept 永久阻塞无法退出
        self._server = server

        while True:
            if should_stop is not None and should_stop():
                self.close()
                raise ConnectionError("等待 PLC 连接被中止")
            try:
                client, address = server.accept()
                break
            except socket.timeout:
                continue
        client.settimeout(None)  # 数据收发超时由 receive() 按需设置
        self._client = client
